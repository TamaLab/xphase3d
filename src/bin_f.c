...
// To bin a large 3D density model (double) into a smaller one for preview


void print_usage()
{
    printf ("\n");
    printf ("Usage: bin_f BF IN OUT XN YN ZN\n\n");
    printf ("Arguments\n");
...
int main(int argc, char **argv)
{
    ...
    double *slice_R = (double*)malloc(sizeof(double) * yN * zN);
    ptrdiff_t x, y, z;
    ptrdiff_t X, Y, Z;
    ptrdiff_t dx, dy, dz;
    ...
    for (x = 0; x < xN; x++)
    {
        ...
        flag = read_2d_double(filename_R, yN, zN, slice_R);
        if (flag != 0)
        {
            return 1;
        }
